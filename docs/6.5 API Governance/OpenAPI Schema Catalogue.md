...
| Component / file | Purpose | Governing source | Visibility | Dependencies | Lifecycle | Notes |
|---|---|---|---|---|---|---|
| `StructureSummary` / `structureSummary.yaml` | Provides the concise representation of a Structure used for collection and relationship contexts. | API Contract v0.4.3 Structure resource, PES applicability and collection provisions; authoritative Structures dataset | Public | `Identifier` | Review | Contains resource identity, display information and governed Structure classification. Detailed construction/exposure applicability and orientation information remain on the complete Structure representation. `category` supports Structure browsing and identification of PES-eligible Structures without duplicating the governing applicability rule. |
...
| `HazardCategorySummary` / `hazardCategorySummary.yaml` | Provides the concise representation of a Hazard Category used for collection and relationship contexts. | API Contract v0.4.3 Hazard Category resource and collection provisions; authoritative Hazard Categories dataset | Public | `Identifier` | Review | Contains resource identity, classification, parent-division relationship and active state for browsing. Description, effects, supported quantity bases and provenance remain on the complete Hazard Category representation. Controlled hazard-category types remain governed by authoritative data. |
| `EcmProtectionRatingSummary` / `ecmProtectionRatingSummary.yaml` | Provides the concise representation of an ECM Protection Rating used for collection and relationship contexts. | API Contract v0.4.3 ECM Protection Rating resource and collection provisions; authoritative ECM Protection Ratings dataset | Public | `Identifier` | Review | Contains resource identity and display information only. Description and authoritative provenance remain on the complete ECM Protection Rating representation. Controlled rating values remain governed by authoritative data. |
...
| `HazardCategory` / `hazardCategory.yaml` | Represents an authoritative Hazard Category used within the validated engineering context, including its classification, associated effects and supported quantity bases. | API Contract v0.4.3 Hazard Category resource and entity-retrieval provisions; authoritative Hazard Categories dataset | Public | `Identifier`, `SourceReference` | Review | `parentDivision` is a governed Hazard Division code or `null`, not a resource ID. Effect references use stable identifiers. Hazard type and supported quantity-basis vocabularies remain governed by authoritative data and are not duplicated as OpenAPI enums. |
...
| Component / file | Purpose | Governing source | Visibility | Dependencies | Lifecycle | Notes |
|---|---|---|---|---|---|---|
| `StructureCollection` / `structureCollection.yaml` | Envelopes the concise public collection of Structures with common response metadata. | API Contract v0.4.3 Structure collection and successful-response provisions | Public | `ResponseMetadata`, `StructureSummary` | Review | The `data` array uses the approved Structure summary projection. Detailed construction/exposure applicability and orientation information remain on individual Structure resources. Pagination and collection counts are outside the MVP contract. |
...
| `HazardCategoryCollection` / `hazardCategoryCollection.yaml` | Envelopes the concise public collection of Hazard Categories with common response metadata. | API Contract v0.4.3 Hazard Category collection and successful-response provisions | Public | `ResponseMetadata`, `HazardCategorySummary` | Review | The `data` array uses the approved Hazard Category summary projection. Detailed engineering description, effects, supported quantity bases and provenance remain on individual Hazard Category resources. Pagination and collection counts are outside the MVP contract. |
| `EcmProtectionRatingCollection` / `ecmProtectionRatingCollection.yaml` | Envelopes the concise public collection of ECM Protection Ratings with common response metadata. | API Contract v0.4.3 ECM Protection Rating collection and successful-response provisions | Public | `ResponseMetadata`, `EcmProtectionRatingSummary` | Review | The `data` array uses the approved ECM Protection Rating summary projection. Description and authoritative provenance remain on individual ECM Protection Rating resources. Pagination and collection counts are outside the MVP contract. |
